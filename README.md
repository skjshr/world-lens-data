...
| `snapshots/lbma-commodities.json` | LBMA（金 PM / 銀 fix） | 15 分 | 不要 |
...
### 貴金属について
FRED の金銀連動シリーズ（GOLDPMGBD228NLBM / SLVPRUSD 等）は 2020 年前後の LBMA ポリシー変更
以降で事実上停止している。代替として prices.lbma.org.uk 直下の公開 JSON endpoint を叩く。
`gold_pm.json` / `silver.json` は全履歴配列を返し、配列末尾が最新 fix。v=[USD, GBP, EUR]。
前日比は endpoint 内の前日値との差分で計算するので Stooq のような prev cache は不要。
